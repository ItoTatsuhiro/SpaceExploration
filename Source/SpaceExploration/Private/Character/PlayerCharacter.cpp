...
void APlayerCharacter::ClickedMouseLeftButton()
{
	...
	if (PlayerController->DeprojectMousePositionToWorld(WorldLocation, WorldDirection)) {
		...
		if (GetWorld()->LineTraceSingleByChannel(HitResult, Start, End, ECC_Visibility))
		{
			...
			if (!MouseButtonEvent)
			{
				UKismetSystemLibrary::PrintString(this, "IMouseButtonEventの取得に失敗しました。", true, true, FColor::Yellow, 2.f, TEXT(""));
				UE_LOG(LogTemp, Warning, TEXT("IMouseButtonEventの取得に失敗しました。"), nullptr);
				return;
			}
